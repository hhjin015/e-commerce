...
public class ProductItemCommandController {
    ...
    public ResponseEntity<Void> modifyProductItem(@Valid @RequestBody ModifyProductItemRequest request) {
        String op = request.getOperation();

        switch (op) {
            case "ADD" -> {
                productItemCommandService.addProductItem(request.getProductId(), request.productItemToData());
            }
            case "REMOVE" -> {
                productItemCommandService.deleteProductItem(request.getRemoveIds());
            }
            case "MODIFY" -> {
                productItemCommandService.modifyProductItem(request.modifyProductItemToData());
            }
        }
